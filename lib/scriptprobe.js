...
ScriptTest.prototype.go = function(callback) {
    ...
    var me = this;
    spbrowser.loadScriptInBrowser(serverHostname, url, remoteServer, function(results) {
        me.callback(results);
    })
...
if (!module.parent) {
    ...
    var test = new ScriptTest(program.url, program.remote);
    test.go(function(results) {
        ...
        console.log(results.httpRequests[0].gzip);
    });
